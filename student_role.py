...
role = st.sidebar.selectbox("Select Role", ["advisor", "chair", "student"])
...
if user_id:
    ...
    def generate_course_tips(course_id):
        ...
        tips = [
            f"<li>Review the concept: <b>{kw}</b></li>" for kw in keywords[:3]
        ] + [
            "<li>Watch lecture recordings or related videos on these topics.</li>",
            "<li>Practice with previous assignments or quiz questions.</li>",
            "<li>Reach out to your instructor or a peer tutor for clarification.</li>",
            "<li>Set aside daily focused time for this course topic.</li>"
        ]
        return '<ul style="margin-bottom: 20px;">' + ''.join(tips) + '</ul>'
    ...
    with tab2:
        ...
        st.markdown("### 🧠 AI Study Tips for High-Risk Courses")
        high_risk_student_courses = selected_courses[selected_courses['predicted_risk'] == 'High']['course_id'].unique()
        filtered_tips = [
            {"course_id": cid, "ai_study_tips": generate_course_tips(cid)}
            for cid in high_risk_student_courses if cid in lms_df['course_id'].values
        ]
        if filtered_tips:
            for tip in filtered_tips:
                st.markdown(f"<b>{tip['course_id']}</b>" + tip['ai_study_tips'], unsafe_allow_html=True)
        else:
            st.info("No high-risk courses with study tips available for this student.")
